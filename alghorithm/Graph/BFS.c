/*
 * brief@ Graph Traversal and Minimum Distance between Source and Destination node.
 * author@ devsks
...
void BFS( graph **G, int root,int *paren, int *dist)
{
    int queue[123], front, rear, vis[123] = {0}, u, v;
    front = rear = -1;
    queue[rear++, ++front] = root;
    vis[root] = 1;
    while(front <= rear)
    {
        v = queue[front++];
